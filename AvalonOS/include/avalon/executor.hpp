...
#ifndef __AVALON_EXECUTOR_HPP__
#define __AVALON_EXECUTOR_HPP__
...
/**
 * Avalon executor interface and executor driver. An executor is
 ...
 * a new AvalonExecutorDriver (also below). In fact, while a Avalon
 ...
 * runs the AvalonSchedulerDriver. The only contract with the slave is
 ...
 * src/java/src/org/apache/avalon, Python: src/python/src, etc.).
 */
...
public:
   ...
   * connect with Avalon. In particular, a scheduler can pass some
   ...
   * running tasks. Note that after a Avalon has determined that an
...
/**
 * Abstract interface for connecting an executor to Avalon. This
 * interface is used both to manage the executor's lifecycle (start
 * it, stop it, or wait for it to finish) and to interact with Avalon
 * (e.g., send status updates, send framework messages, etc.). See
 * AvalonExecutorDriver below for a concrete example of an
 * ExecutorDriver.
 */
class ExecutorDriver
{
public:
   ...
   * (see avalon.proto for a description of Status).
   */
 ...
 * Executor with a Avalon slave. The AvalonExecutorDriver is thread-safe.
 *
 * The driver is responsible for invoking the Executor callbacks as it
 * communicates with the Avalon slave.
 *
 * Note that blocking on the AvalonExecutorDriver (e.g., via
 * AvalonExecutorDriver::join) doesn't affect the executor callbacks in
 * anyway because they are handled by a different thread.
 *
 * See src/examples/test_executor.cpp for an example of using the
 * AvalonExecutorDriver.
 */
class AvalonExecutorDriver : public ExecutorDriver
{
public:
  /**
   ...
   */
  AvalonExecutorDriver(Executor* executor);

  /**
   * This destructor will block indefinitely if
   * AvalonExecutorDriver::start was invoked successfully (possibly via
   * AvalonExecutorDriver::run) and AvalonExecutorDriver::stop has not
   * been invoked.
   */
  virtual ~AvalonExecutorDriver();
