...
#ifndef __AVALON_SCHEDULER_HPP__
#define __AVALON_SCHEDULER_HPP__
...
/**
 * Avalon scheduler interface and scheduler driver. A scheduler is used
 * to interact with Avalon in order run distributed computations.
 *
...
public:
  ...
  /**
   * Invoked when the scheduler successfully registers with a Avalon
  ...
  /**
   * Invoked when the scheduler re-registers with a newly elected Avalon master.
...
/**
 * Abstract interface for connecting a scheduler to Avalon. This
 * interface is used both to manage the scheduler's lifecycle (start
 * it, stop it, or wait for it to finish) and to interact with Avalon
 * (e.g., launch tasks, kill tasks, etc.). See AvalonSchedulerDriver
...
public:
   ...
   * reconnect to Avalon and all of it's executors and tasks can be
  ...
  /**
   * Requests resources from Avalon (see avalon.proto for a description
   ...
   * resources (see avalon.proto for a description of Filters).
   ...
   * filters on the resources (see avalon.proto for a description of
 ...
 * Scheduler with a Avalon master. The AvalonSchedulerDriver is
 * thread-safe.
 *
 * Note that scheduler failover is supported in Avalon. After a
 * scheduler is registered with Avalon it may failover (to a new
 ...
 * it communicates with the Avalon master.
 *
 * Note that blocking on the AvalonSchedulerDriver (e.g., via
 * AvalonSchedulerDriver::join) doesn't affect the scheduler callbacks
 * in anyway because they are handled by a different thread.
 *
 * See src/examples/test_framework.cpp for an example of using the
 * AvalonSchedulerDriver.
 */
class AvalonSchedulerDriver : public SchedulerDriver
{
public:
  /**
   ...
   *
   * Any Avalon configuration options are read from environment
   * variables, as well as any configuration files found through the
   * environment variables.
   */
  AvalonSchedulerDriver(Scheduler* scheduler,
                       const FrameworkInfo& framework,
                       const std::string& master);

  /**
   * This destructor will block indefinitely if
   * AvalonSchedulerDriver::start was invoked successfully (possibly
   * via AvalonSchedulerDriver::run) and AvalonSchedulerDriver::stop has
   * not been invoked.
   */
  virtual ~AvalonSchedulerDriver();
...
#endif // __AVALON_SCHEDULER_HPP__
