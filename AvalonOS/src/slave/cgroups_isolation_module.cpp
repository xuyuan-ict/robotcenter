    ...
    const PID<Slave>& _slave)
{
  ...
  // Create the root "avalon" cgroup if it doesn't exist.
  Try<bool> exists = cgroups::exists(hierarchy, "avalon");
  CHECK_SOME(exists)
    << "Failed to determine if the 'avalon' cgroup already exists "
    << "in the hierarchy at '" << hierarchy << "'";
  if (!exists.get()) {
    // No root cgroup exists, create it.
    Try<Nothing> create = cgroups::create(hierarchy, "avalon");
    CHECK_SOME(create) << "Failed to create the 'avalon' cgroup";
  }

  // Make sure this kernel supports creating nested cgroups.
  Try<Nothing> create = cgroups::create(hierarchy, "avalon/test");
  if (create.isError()) {
  ...
  Try<Nothing> remove = cgroups::remove(hierarchy, "avalon/test");
  ...
  Try<int> fd = os::open(path::join(hierarchy, "avalon", "tasks"), O_RDONLY);
  CHECK_SOME(fd);
  Try<Nothing> cloexec = os::cloexec(fd.get());
  CHECK_SOME(cloexec);
  if (flock(fd.get(), LOCK_EX | LOCK_NB) != 0) {
    EXIT(1) << "Another avalon-slave appears to be running!";
  }
  ...
  Try<vector<string> > cgroups = cgroups::get(hierarchy, "avalon");
  CHECK_SOME(cgroups) << "Failed to get nested cgroups of 'avalon'";
  ...
  exists = cgroups::exists(hierarchy, "avalon", "memory.oom_control");
  CHECK_SOME(exists)
  ...
  Try<Nothing> write = cgroups::write(
      hierarchy, "avalon", "memory.oom_control", "1");
  ...
  if (subsystems.contains("cpuset")) {
    ...
    Try<string> cpuset = cgroups::read(hierarchy, "avalon", "cpuset.cpus");
