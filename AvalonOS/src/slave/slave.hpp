...
struct Framework
{
  ...
  ExecutorInfo getExecutorInfo(const TaskInfo& task)
  {
    ...
    if (task.has_command()) {
      ...
      // update it to invoke 'avalon-executor' (unless we couldn't
      // resolve 'avalon-executor' via 'realpath', in which case just
      // echo the error and exit).
      executor.mutable_command()->MergeFrom(task.command());

      Try<std::string> path = os::realpath(
          path::join(flags.launcher_dir, "avalon-executor"));
