...
Resource Resources::parse(const std::string& name, const std::string& text, const int type)
{
  Resource resource;
  ...
  if (result.isError()) {
    ...
    switch(type) {
      case 0 : resource.set_rtype(CRES); break;
      case 1 : resource.set_rtype(DRES); break;
      case 2 : resource.set_rtype(ARES); break;
    }
  }
...
Resources Resources::parse(const string& s, int type)
{
  ...
  for (size_t i = 0; i < tokens.size(); i++) {
    ...
    resources += parse(pairs[0], pairs[1], type);
  }
