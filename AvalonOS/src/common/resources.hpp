...
// resources easier within the Avalon core we provide generic
...
public:
  ...
  static Resource parse(const std::string& name, const std::string& value, const int type);
  static Resources parse(const std::string& s, const int type);

  static Resources updateRes(const Resources& origin, const Resources& used) {
    Resources result;
    bool containAres = false;

    foreach (const Resource& resource, used.resources) {
      if (resource.rtype() == CRES)
        result += resource;
      else if (resource.rtype() == ARES)
        containAres = true;
    }

    if(containAres) {
      foreach (const Resource& resource, origin.resources) {
        if (resource.rtype() == ARES)
          result += resource;
      }
    }
...
{
  avalon::internal::Resources::const_iterator it = resources.begin();
