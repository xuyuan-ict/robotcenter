    ...
    bool verbose)
{
  ...
  if (os::hasenv("AVALON_KILLTREE")) {
    // Set in avalon-slave-flags.sh.
    cmdline = os::getenv("AVALON_KILLTREE");
  } else if (os::hasenv("AVALON_SOURCE_DIR")) {
    // Set by test harness for external tests.
    cmdline = os::getenv("AVALON_SOURCE_DIR") +
