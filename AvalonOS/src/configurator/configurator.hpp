 ...
 * (i) Environment variables. It adds all variables starting with AVALON_.
 * (ii) Command line variables. Supports "--key=val" "-key val" "-opt" "--opt"
 * (iii) Config file. It ignores comments "#". It finds the file avalon.conf
...
public:
   ...
   * It picks all environment variables that start with AVALON_.
   * The environment var AVALON_FOO=/dir would lead to key=foo val=/dir<br>
   *
   * <i>Command line:</i><br>
   ...
   * Parses the environment variables and populates a Configuration.
   * It picks all environment variables that start with AVALON_.
   * The environment var VALON_FOO=/dir would lead to key=foo val=/dir <br>
   *
   * <i>Config file:</i><br>
   * The config file should contain key=value pairs, one per line.
   * Comments, which should start with #, are ignored.
  ...
  /**
   * Clears all Avalon environment variables (useful for tests).
   */
  static void clearAvalonEnvironmentVars();

private:
  /**
   * Parses the environment variables and populates a Configuration.
   * It picks all environment variables that start with AVALON_.
   * The environment variable AVALON_FOO=/dir would lead to key=foo val=/dir
