...
// the master and responding to Avalon API calls from scheduler
...
{
public:
  SchedulerProcess(AvalonSchedulerDriver* _driver,
...
protected:
  ...
  void doReliableRegistration()
  {
    ...
    if (!framework.has_id() || framework.id() == "") {
      ...
      // send(master, message);
      VLOG(1) << "Framework needs reregistered!!!";
    }
...
//   }
  ...
  // AvalonSchedulerDriver::stop later, which dispatches to
...
private:
  friend class avalon::AvalonSchedulerDriver;

  AvalonSchedulerDriver* driver;
  Scheduler* scheduler;
...
AvalonSchedulerDriver::AvalonSchedulerDriver(
    Scheduler* _scheduler,
    const FrameworkInfo& _framework,
...
AvalonSchedulerDriver::~AvalonSchedulerDriver()
{
  ...
  // AvalonSchedulerDriver::stop was never called. It might make sense
...
Status AvalonSchedulerDriver::start()
{
  Lock lock(&mutex);
...
Status AvalonSchedulerDriver::stop(bool failover)
{
  Lock lock(&mutex);
  ...
  // ours can just be an instance variable in AvalonSchedulerDriver.
...
Status AvalonSchedulerDriver::abort()
{
  Lock lock(&mutex);
...
Status AvalonSchedulerDriver::join()
{
  Lock lock(&mutex);
...
Status AvalonSchedulerDriver::run()
{
  Status status = start();
...
// Status AvalonSchedulerDriver::killTask(const TaskID& taskId)
// {
//   Lock lock(&mutex);
...
Status AvalonSchedulerDriver::launchTasks(
...
// Status AvalonSchedulerDriver::declineOffer(
//     const OfferID& offerId,
//     const Filters& filters)
// {
...
// Status AvalonSchedulerDriver::reviveOffers()
// {
//   Lock lock(&mutex);
...
// Status AvalonSchedulerDriver::sendFrameworkMessage(
//     const ExecutorID& executorId,
//     const SlaveID& slaveId,
//     const string& data)
// {
...
// Status AvalonSchedulerDriver::requestResources(
