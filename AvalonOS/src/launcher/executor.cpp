...
// AvalonSchedulerDriver for how we "get around" any issues related to
...
{
  avalon::internal::CommandExecutor executor;

  // Note that we currently put the AvalonSchedulerDriver on the heap
  // so that we don't have to deal with issues created because the
  // thread we launched is trying to use the pointer.
  avalon::AvalonExecutorDriver* driver =
    new avalon::AvalonExecutorDriver(&executor);

  return driver->run() == avalon::DRIVER_STOPPED ? 0 : 1;
}
