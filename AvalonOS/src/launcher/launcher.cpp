...
int ExecutorLauncher::setup()
{
  ...
  // if (redirectIO) {cout << "0" << endl;
  //   if (freopen("stdout", "w", stdout) == NULL) {cout << "1" << endl;
  //     fatalerror("freopen failed");
  //   }cout << "2" << endl;
  //   if (freopen("stderr", "w", stderr) == NULL) {cout << "3" << endl;
  //     fatalerror("freopen failed");
  //   }
  // }cout << "0-4" << endl;
...
{
  cout << "**** ExecutorLauncher::run ****" << endl;
  int ret = setup();
  if (ret < 0) {
    return ret;
  }
...
int ExecutorLauncher::fetchExecutors()
{
  ...
  foreach(const CommandInfo::URI& uri, commandInfo.uris()) {
    ...
    if (resource.find("hdfs://") == 0 || resource.find("hftp://") == 0) {
      ...
      // the slave (from the Avalon config file), use that. Otherwise check for
...
void ExecutorLauncher::setupEnvironment()
{
  ...
  // Also add AVALON_NATIVE_LIBRARY if it's not already present (and
  // like above, we do this before the environment variables below in
  // case the framework wants to override).
  if (!os::hasenv("AVALON_NATIVE_LIBRARY")) {
    string path =
#ifdef __APPLE__
      LIBDIR "/libavalon-" VERSION ".dylib";
#else
      LIBDIR "/libavalon-0.1.so";
      // LIBDIR "/libavalon-" VERSION ".so";
#endif
    if (os::exists(path)) {
      os::setenv("AVALON_NATIVE_LIBRARY", path);
    }
  }
  ...
  // Set Avalon environment variables for slave ID, framework ID, etc.
  os::setenv("AVALON_DIRECTORY", workDirectory);
  os::setenv("AVALON_SLAVE_PID", slavePid);
  os::setenv("AVALON_FRAMEWORK_ID", frameworkId.value());
  os::setenv("AVALON_EXECUTOR_ID", executorId.value());
}
...
void ExecutorLauncher::setupEnvironmentForLauncherMain()
{
  ...
  // Set up Avalon environment variables that launcher/main.cpp will
  ...
  string file_name = "slaveConf";
  string swwitch_user = shouldSwitchUser ? "1" : "0";
  string redirect_io = redirectIO ? "1" : "0";
  os::touch(file_name);
  int fd = os::open(file_name, O_RDWR).get();
  os::write(fd, frameworkId.value()+"\n");
  os::write(fd, executorId.value()+"\n");
  os::write(fd, commandInfo.value()+"\n");
  os::write(fd, uris+"\n");
  os::write(fd, user+"\n");
  os::write(fd, workDirectory+"\n");
  os::write(fd, slavePid+"\n");
  os::write(fd, hadoopHome+"\n");
  os::write(fd, redirect_io+"\n");
  os::write(fd, swwitch_user+"\n");
  os::write(fd, container);
  // os::close(fd);

  os::setenv("AVALON_FRAMEWORK_ID", frameworkId.value());
  os::setenv("AVALON_COMMAND", commandInfo.value());
  os::setenv("AVALON_EXECUTOR_URIS", uris);
  os::setenv("AVALON_USER", user);
  os::setenv("AVALON_WORK_DIRECTORY", workDirectory);
  os::setenv("AVALON_SLAVE_PID", slavePid);
  os::setenv("AVALON_HADOOP_HOME", hadoopHome);
  os::setenv("AVALON_REDIRECT_IO", redirectIO ? "1" : "0");
  os::setenv("AVALON_SWITCH_USER", shouldSwitchUser ? "1" : "0");
  os::setenv("AVALON_CONTAINER", container);
}
